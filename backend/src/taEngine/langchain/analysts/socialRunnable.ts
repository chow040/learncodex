...
export const buildSocialCollaborationHeader = (context: AnalystNodeContext): string => {
  ...
  const systemPrompt = context.systemPrompt ?? SOCIAL_SYSTEM_PROMPT;
  return `You are a helpful AI assistant, collaborating with other assistants. Use the provided tools to progress towards answering the question. If you are unable to fully answer, that's OK; another assistant with different tools will help where you left off. Execute what you can to make progress. If you or any other assistant has the FINAL TRANSACTION PROPOSAL: **BUY/HOLD/SELL** or deliverable, prefix your response with FINAL TRANSACTION PROPOSAL: **BUY/HOLD/SELL** so the team knows to stop. You have access to the following tools: ${toolList}.\n${systemPrompt}For your reference, the current date is ${context.tradeDate}. The current company we want to analyze is ${context.symbol}`;
};
...
export const buildSocialUserContext = (context: AgentsContext): string => {
  ...
  if (socialBuzz) {
    ...
    sections.push(buildMissingSection('Social stock buzz', 'get_reddit_news'));
  }
