...
const buildFundamentalsSummary = (
  profile: CompanyProfile,
  metrics: StockMetrics,
): { summary: string; balance: string; cashflow: string; income: string } => {
  const lines = [
    ...
    `Market cap: ${formatCurrency(profile.marketCapitalization, profile.currency || 'USD')} | Shares outstanding: ${formatNumber(profile.shareOutstanding)}`,
...
const buildPayload = async (symbol: string): Promise<{ payload: any; decisionDate: string }> => {
...
const invokeTradingAgentsRunner = async (payload: any): Promise<TradingAgentsDecision> => {
  ...
  try {
    ...
    return resp.data as TradingAgentsDecision;
  } catch (err) {
...
export const requestTradingAgentsDecision = async (symbol: string): Promise<TradingAgentsDecision> => {
