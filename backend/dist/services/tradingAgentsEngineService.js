...
export const requestTradingAgentsDecisionInternal = async (symbol) => {
        ...
        if (env.finnhubApiKey) {
            ...
            try {
                ...
                if (tx.length) {
                    const lines = tx
                        .slice(0, 12)
                        .map((t) => `• ${t.transactionDate} ${t.name} (${t.transactionCode}) @ ${t.transactionPrice ?? 'N/A'} | Change ${t.change ?? 'N/A'} | Shares ${t.share ?? 'N/A'}`);
            ...
            try {
                ...
                globalThis._la_gnews = gFormatted;
            }
            catch (e) {
    ...
    const gnews = globalThis._la_gnews;
