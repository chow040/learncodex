...
const truncate = (text, max = 320) => text.length <= max ? text : `${text.slice(0, max).trimEnd()}�`;
const buildSupplementalContext = (context) => {
    if (!context) {
        return null;
    }
    const sections = [];
...
export const requestEquityAssessment = async (input, context) => {
