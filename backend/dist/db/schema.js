...
export const agentContextPolicies = pgTable('agent_context_policies', {
    id: uuid('id').defaultRandom().primaryKey(),
    agentId: uuid('agent_id')
        .references(() => agents.id, { onDelete: 'cascade' })
        .notNull()
...
export const agentRuns = pgTable('agent_runs', {
    ...
    tickers: jsonb('tickers').$type().notNull(),
    question: text('question'),
    status: agentRunStatusEnum('status').default('running').notNull(),
    decisionSummary: text('decision_summary'),
    confidence: numeric('confidence', { precision: 5, scale: 4 }).$type(),
...
export const agentRunSnapshots = pgTable('agent_run_snapshots', {
    id: uuid('id').defaultRandom().primaryKey(),
    runId: uuid('run_id')
    ...
    parsedOutputJson: jsonb('parsed_output_json').$type(),
