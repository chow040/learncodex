export class FundamentalsAnalyst {
    analyze(ctx, symbol, tradeDate) {
        ...
        const userLines = [];
        if (ctx.fundamentals_summary)
            userLines.push(`Fundamentals overview:\n${ctx.fundamentals_summary}`);
        if (ctx.fundamentals_balance_sheet)
            userLines.push(`Balance sheet:\n${ctx.fundamentals_balance_sheet}`);
        if (ctx.fundamentals_cashflow)
            userLines.push(`Cash flow:\n${ctx.fundamentals_cashflow}`);
        if (ctx.fundamentals_income_stmt)
            userLines.push(`Income statement:\n${ctx.fundamentals_income_stmt}`);
        if (ctx.fundamentals_insider_transactions)
            userLines.push(`Insider transactions (recent):\n${ctx.fundamentals_insider_transactions}`);
        return {
...
//# sourceMappingURL=FundamentalsAnalyst.js.map