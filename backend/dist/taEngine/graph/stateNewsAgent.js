...
const sanitizeNewsValue = (value) => {
    ...
    if (KNOWN_PLACEHOLDERS_LOWER.has(lower))
        return FALLBACK_NO_DATA;
    if (PLACEHOLDER_HINTS.some((hint) => lower.includes(hint)))
        return FALLBACK_NO_DATA;
    return str;
};
const parseInteger = (value, fallback, opts = {}) => {
...
const formatEpochDate = (epochSeconds) => {
...
const formatCompanyNewsArticles = (articles, limit) => {
    if (!articles.length) {
...
const formatGoogleNewsItems = (items, limit) => {
    ...
    const valid = items
        ...
        const metadataParts = [];
        if (source)
            metadataParts.push(`Source: ${source}`);
        if (date)
            metadataParts.push(`Published: ${date}`);
...
const formatRedditInsights = (insights, limit) => {
    if (!insights) {
...
export class StateNewsAgent {
    ...
    createToolHandlers(context, symbol, tradeDate) {
        ...
        const shouldFetch = (existing, forceRefresh) => existing === FALLBACK_NO_DATA || forceRefresh;
        return {
            get_finnhub_news: async (rawArgs = {}) => {
            ...
            get_google_news: async (rawArgs = {}) => {
                ...
                try {
                    ...
                    const formatted = formatGoogleNewsItems(results, limit);
    ...
    async callModel(messages, includeTools = false, timeoutMs = FOLLOWUP_AI_TIMEOUT_MS) {
        const params = {
        ...
        return Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('AI call timeout')), timeoutMs)),
        ]);
    }
...
//# sourceMappingURL=stateNewsAgent.js.map