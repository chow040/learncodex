...
const parseTradeDate = (tradeDate) => {
    if (!tradeDate)
        return new Date();
    const parsed = new Date(tradeDate);
    return Number.isNaN(parsed.getTime()) ? new Date() : parsed;
};
const resolveTicker = (inputTicker, context) => {
...
const formatCompanyNews = (articles, limit, ticker) => {
    if (!articles.length) {
...
const formatRedditNews = (insights, limit) => {
...
export const registerNewsTools = () => {
    ...
    registerTool({
        ...
        create: (context) => new DynamicStructuredTool({
            ...
            func: async (input) => withToolLogging(TOOL_IDS.GOOGLE_NEWS, input, context.logger, async () => {
    ...
    registerTool({
        ...
        create: (context) => new DynamicStructuredTool({
            ...
            func: async (input) => withToolLogging(TOOL_IDS.FINNHUB_MARKET_NEWS, input, context.logger, async () => {
    ...
    registerTool({
        ...
        create: (context) => new DynamicStructuredTool({
            ...
            func: async (input) => withToolLogging(TOOL_IDS.REDDIT_NEWS, input, context.logger, async () => {
                const { ticker, warning } = resolveTicker(input.ticker, context);
...
//# sourceMappingURL=newsTools.js.map