...
import { assignPromptProfileToAgent, createPromptProfile, getAgentConfiguration, getPromptProfile, listAgentSummaries, listPromptProfiles, listSystemSettingsByScope, previewAgentPrompt, updateAgentConfiguration, updatePromptProfile, upsertSystemSettings, } from '../services/adminDashboardService.js';
import { agentUpdateSchema, assignPromptSchema, createPromptProfileSchema, promptPreviewSchema, promptProfileQuerySchema, systemSettingsPatchSchema, updatePromptProfileSchema, } from '../validators/adminDashboard.js';
...
const normalizeArrayPayload = (body) => {
...
adminRouter.patch('/system-settings', async (req, res, next) => {
    try {
        const updates = systemSettingsPatchSchema.parse(normalizeArrayPayload(req.body));
...
adminRouter.patch('/agents/:id', async (req, res, next) => {
    try {
        ...
        const payload = agentUpdateSchema.parse(req.body ?? {});
...
adminRouter.post('/agents/:id/preview-prompt', async (req, res, next) => {
    try {
        ...
        const payload = promptPreviewSchema.parse(req.body ?? {});
...
adminRouter.post('/prompt-profiles', async (req, res, next) => {
    try {
        const payload = createPromptProfileSchema.parse(req.body ?? {});
...
adminRouter.patch('/prompt-profiles/:id', async (req, res, next) => {
    try {
        ...
        const payload = updatePromptProfileSchema.parse(req.body ?? {});
