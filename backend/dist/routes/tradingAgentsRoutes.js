...
import { executeAgentRun, getAgentRunDetail, getTradingAgentDetail, listAgentRunsForUser, listPublicTradingAgents, TradingAgentsUserError, } from '../services/tradingAgentsUserService.js';
...
tradingAgentsRouter.get('/:agentId', async (req, res, next) => {
    try {
...
tradingAgentsRouter.post('/:agentId/run', async (req, res, next) => {
    try {
        const agentId = req.params.agentId;
        if (!agentId) {
            return res.status(400).json({ error: 'agentId is required' });
...
tradingAgentsRouter.get('/:agentId/runs', async (req, res, next) => {
    try {
        const agentId = req.params.agentId;
        if (!agentId) {
            return res.status(400).json({ error: 'agentId is required' });
...
tradingAgentsRouter.get('/:agentId/runs/:runId', async (req, res, next) => {
    try {
        const agentId = req.params.agentId;
...
//# sourceMappingURL=tradingAgentsRoutes.js.map