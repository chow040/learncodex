...
import { conversationMessages, conversationSessions, conversationSummaries, reports } from '../db/schema.js';
import { CONVERSATION_DEFAULT_WINDOW, CONVERSATION_MAX_WINDOW, CONVERSATION_RETENTION_DAYS, CONVERSATION_SUMMARY_ANCHOR_COUNT, CONVERSATION_SUMMARY_BYTE_THRESHOLD, CONVERSATION_SUMMARY_MAX_CHARS, CONVERSATION_SUMMARY_MESSAGE_THRESHOLD, MAX_CONVERSATION_MESSAGE_BYTES, MAX_CONVERSATION_TOTAL_BYTES } from '../config/limits.js';
...
const ensureReportOwnership = async (reportId, userId) => {
...
const getOrCreateSession = async (reportId, userId, model) => {
...
const getStorageUsage = async (reportId, userId) => {
    const [messageBytes] = await db.select({
        total: sql `COALESCE(SUM(octet_length(${conversationMessages.content})), 0)`
    ...
    const [summaryBytes] = await db.select({
        total: sql `COALESCE(SUM(octet_length(${conversationSummaries.summary})), 0)`
...
const fetchMessagesAsc = async (reportId, userId) => {
...
const upsertSummary = async (reportId, sessionId, summary, coverageUpTo) => {
...
const pruneOldMessages = async (reportId, userId, coverageUpTo) => {
    if (!coverageUpTo)
        return;
    await db.delete(conversationMessages)
        .where(and(eq(conversationMessages.reportId, reportId), eq(conversationMessages.userId, userId), lte(conversationMessages.createdAt, coverageUpTo)));
};
const applyRetention = async (reportId, userId) => {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - CONVERSATION_RETENTION_DAYS);
    await db.delete(conversationMessages)
        .where(and(eq(conversationMessages.reportId, reportId), eq(conversationMessages.userId, userId), lte(conversationMessages.createdAt, cutoff)));
};
export const summarizeIfNeeded = async (reportId, userId) => {
    await ensureReportOwnership(reportId, userId);
...
export const getConversation = async (reportId, userId, limit) => {
    await ensureReportOwnership(reportId, userId);
