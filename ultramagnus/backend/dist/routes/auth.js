...
const ensureProfile = async (userId, email, displayName) => {
...
authRouter.post('/google/exchange', async (req, res) => {
    ...
    try {
        const decoded = jwt.verify(oauth_token, config.sessionSecret);
...
authRouter.get('/google/callback', async (req, res) => {
    ...
    try {
        ...
        const tempToken = jwt.sign({ userId: user.id, email: user.email }, config.sessionSecret, { expiresIn: '5m' });
